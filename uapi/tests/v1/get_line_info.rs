...
fn check_info() {
    ...
    let mut h = get_line_handle(fd, hr).unwrap();
    ...
    h = get_line_handle(fd, hr).unwrap();
...
fn with_bad_fd() {
    ...
    let fd = f.as_raw_fd();
    drop(f);
    assert_eq!(get_line_info(fd, 1), Err(Error::Os(Errno(libc::EBADF))));
}
