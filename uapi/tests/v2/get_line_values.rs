...
fn on_input() {
    ...
    values.unset_mask(2);
    s.set_pull(0, Level::High).expect("set_pull should succeed");
    s.set_pull(1, Level::Low).expect("set_pull should succeed");
    s.set_pull(2, Level::High).expect("set_pull should succeed");
    s.set_pull(3, Level::Low).expect("set_pull should succeed");
    wait_propagation_delay();
...
fn with_no_values() {
    ...
    assert_eq!(
        get_line_values(&l, &mut values).expect_err("get_line_values should fail"),
